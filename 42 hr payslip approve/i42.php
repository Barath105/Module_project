...
<?php
        ...
        $sql = "SELECT * FROM hrpayslip WHERE status = 'Pending'";
    ...
    <img src="logo.png">
            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $id = $row['id'];
                        $userid = $row['userid'];
        $username = $row['username']; // Corrected variable name
        $request_on = $row['request_on']; // Corrected variable name
        $date = $row['date']; // Corrected variable name
        $status = $row['status'];
        ...
        echo '</td>';
        echo '<td>';
        echo '<a href="../54 payslip generator/payroll.php?id=' . $id . '&username=' . urlencode($username) . '&userid=' . $userid . '" class="generate-btn" data-username="' . $username . '" data-userid="' . $userid . '">Generate pdf</a>';
        echo '</td>';
    echo '</tr>';
